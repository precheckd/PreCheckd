...
function parseJsonField(value) {
  if (!value) return [];
  if (Array.isArray(value)) return value; // already parsed, just in case
  try {
...
router.post('/:slug/edit', async (req, res) => {
  try {
    ...
    // Preserve verified status on entries that are unchanged; anything new
    // or edited resets to unverified, since the claim being made has changed.
    function mergeEntries(newEntries, existingEntries, matchFields) {
      ...
      return newEntries
        ...
        .map((e) => {
          ...
          if (existing) {
            return existing; // unchanged — keep prior verification status
          }
          return {
            ...e,
            verified: false,
